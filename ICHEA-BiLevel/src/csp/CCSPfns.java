...
import static csp.CspProcess.bestCOPLowLevelfitnessHistory;
...
class Constraint{
    ...
    private Constraint(){ //disallow default constraint
        ;
    }
}
...
public class CCSPfns{
    ...
    static class TPxFactory{
        private static void execute(int fnNumber){
            switch (fnNumber){
                case 1:
                    TP1();
                    break;
...
//    static double maxCSPval;
    ...
    /**
     * Referencing to {@link CspProcess#CSPsols} 
     */
    ...
    static double curLowerLevelFitness;
    static boolean violatesLowLevelConstraint;
    static int lLimit;
    static int uLimit;
       
    /**
     * Any known best solution. System terminates once the known best is found.
     */
    public static double knownOptSol;
    
    /**
     * Known best Lower level optimal solution for bi-level problems.
     */
    public static double knownOptSolLowerLevel;
    
    
    /**
     * Decimal place (DP) of known best solution to be considered
     */
     ...
     * 
     */
    static boolean tolerateInfeasible;
    ...
    public static void objFn(int fn, final ArrayList<Double> lvals, final ArrayList<Double> lfitness_, 
            final ArrayList<Integer> lconstraints, UserInput luseInput, boolean vioLLCons)
...
//        maxCSPval = CspProcess.maxCSPval;
        ...
        curLowerLevelFitness = Double.NaN;
        knownOptSol = Double.NaN;
        knownOptSolDP = 4;
        violatesLowLevelConstraint = vioLLCons;
////        MAX_TIME = 10; //userInput_.totalConstraints - userInput_.getTotalDecisionVars(); // becuase I take total_constraint as total decision vars + given constraints
        ...
        if(userInput_.isBiLevel){
            TPxFactory.execute(userInput_.tpxFn);
        }else{
            GxFactory.execute(userInput_.gxFn);
        }
        ...
        if(violation.size()>0){
            ...
            for (Constraint cns : consFn) {
                if(cns.isViolated){//violation
                    fit+= Math.abs(cns.val); //how far away??
                }
            }
            fitness_.add(fit);
        }else{
        ...
        fitness_.add(curLowerLevelFitness);
...
//        fitness_.add(curFitness); //fitness_[1]    
        ...
        if(userInput_.isBiLevel){
            if(MyMath.roundN(fitness_.get(0), knownOptSolDP) <= MyMath.roundN(knownOptSol,knownOptSolDP)
                    && violation.size() == 0 && MyMath.roundN(curLowerLevelFitness, knownOptSolDP) <= MyMath.roundN(knownOptSolLowerLevel,knownOptSolDP)
                    && CspProcess.getDynamicTime() >= userInput_.maxDynamicTime){            
...
//            }
    ...
    private static void G1(){
        Double x[] = new Double[userInput_.getTotalDecisionVars()];        
        vals.toArray(x);
        // total vars = 13
    ...
    private static void G2(){
        ...
        final int n = userInput_.getTotalDecisionVars();
...
//        tabuConstraints.get(tabuConstraints.size()-1).add(radius); //last one is radius    
    ...
    private static ArrayList<Integer> totalTabuCostraints(){
        ...
        for (ArrayList<Double> tabus : tabuConstraints) {
            ...
            if(sumSqr < 0){
                totalVios.add((userInput_.getTotalDecisionVars()-1)+t); //max normal violation + t
            }
        }
    ...
    /**
     * Standard Test Problems for Single-Objective Bilevel Optimization.
     * Available in <a href="http://bilevel.org">http://bilevel.org</a><BR>
     * <code>
     * total vars (n) = 20; Upper 10, lower 10<BR>
     * total constraints = 20 parametric and 0 functional <BR>
     * -&infin;&le;x<sub>i</sub>&le;+&infin;, i = [0, 9] <BR>
     * -&pi;&le;x<sub>j</sub>&le;+&pi;, j = [10,19] <BR>
     * </code>
     */
    private static void TP10(){
        Double x[] = new Double[userInput_.getTotalDecisionVars()];
        vals.toArray(x);

        knownOptSol = 0.0; 
        knownOptSolLowerLevel = 1.0;
        lLimit = -5;
        uLimit = 5;
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        knownOptSolDP = (int)Math.round(minDP +(maxDP-minDP)*((CspProcess.getgGen()-1)%CspProcess.getGensEachConstraints())*1.0/CspProcess.getGensEachConstraints());
 
        
        double sum = 0.0;
        for (int i = 0; i < 10; i++) {
            sum += Math.abs(x[i]-1)+Math.abs(x[10+i]);
        }
        ...
        if(violatesLowLevelConstraint){
            t[t.length-1]=1;
        }else{
            t[t.length-1]=-1;
        }
        
        double s1,m1; //sum and multiplications
        s1 = 0;
        for (int i = 0; i < 10; i++) {
            s1+= x[i]*x[i]*x[10+i]*x[10+i]; //(xi.yi)^2
        }
        m1 = 1;
        for (int i = 10; i < 20; i++) {
            m1 *= Math.cos(x[i]*x[i-10]/Math.sqrt(i-9));
        }

        
        curLowerLevelFitness = Math.exp((1+s1/4000 - m1));
        ...
        for (int i = 0; i < consFn.size(); i++) {
            if(consFn.get(i).val >= 0.0){ //dt - ...
                CSPsols.get(lastIdx).set(i, (ArrayList<Double>)vals.clone()); //??? dummy one. same x and diff y  
            }else{
                consFn.get(i).isViolated = true;
                violation.add(i);
            } 
        ...
        //parameteric constraints (variable level constraints)
        for (int i = 0; i < x.length; i++) {
            consFn.add(new Constraint(x[i])); 
        }
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        double s1,s2,m1; //sum and multiplications
        s1 = 0;
        for (int i = 10; i < 20; i++) {
            s1+= x[i]*x[i];
        }
        ...
        curLowerLevelFitness = Math.exp((1+s1/4000 - m1)*s2);
    ...
    /**
     * Bilevel benchmark from "Bilevel Optimization based on Iterative Approximation 
     * of Mappings" by Anukar Sinha et.al. <BR>
     * 
     * <code>
     * total vars (n) = 4; Upper 2, lower 2<BR>
     * total constraints = 4 parametric and 3 functional <BR>
     * 0&le;x<sub>i</sub>&le;50, i = 0,1 <BR>
     * -10&le;x<sub>j</sub>&le;20, j = 2,3 <BR>
     * x1 = x[0], x2 = x[1], y1 = x[2], y2 = x[3]; <BR>
     * </code>
     */
    private static void TP8(){
        Double x[] = new Double[userInput_.getTotalDecisionVars()];
        vals.toArray(x);

        knownOptSol = 0.0; 
        knownOptSolLowerLevel = 100.0;
        lLimit = 0;
        uLimit = 250;
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        double x1 = x[0], x2 = x[1], y1 = x[2], y2 = x[3];
        
        curFitness = Math.abs(2*x1+2*x2-3*y1-3*y2-60);
        violation.clear();
        ...
        curLowerLevelFitness = Math.pow(y1-x1+20, 2)+Math.pow(y2-x2+20, 2);
    ...
    /**
     * <P>Bilevel benchmark from "Bilevel Optimization based on Iterative Approximation 
     * of Mappings" by Anukar Sinha et.al. </P>
     * 
     * <code>
     * total vars (n) = 4; Upper 2, lower 2<BR>
     * total constraints = 4 parametric and 4 functional <BR>
     * 0&le;x<sub>i</sub>&le;x<sub>j</sub>, i = 2,3 and j = 0,1 <BR>
     * 0&le;x<sub>j</sub>, j = 0,1 <BR>
     * x1 = x[0], x2 = x[1], y1 = x[2], y2 = x[3]; <BR>
     * </code>
     */
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        curFitness = -((x1+y1)*(x2+y2))/(1+x1*y1+x2*y2);
        violation.clear();
        ...
        //upper level
        t[2] = Math.pow(x1,2)+Math.pow(x2, 2)-100; //<=0
        t[3] = x1-x2; //<=0
        ...
        curLowerLevelFitness = -curFitness;
     ...
     * 
     * <code>
     * total vars (n) = 5; Upper 2, lower 3<BR>
     * total constraints = 5 parametric and 3 functional <BR>
     * 0&le;x<sub>i</sub>, i = 2,3,4 <BR>
     * 0&le;x<sub>j</sub>, j = 0,1 <BR>
     * x1 = x[0], x2 = x[1], y1 = x[2], y2 = x[3], y3 = x[4];<BR>
     * </code>
     */
    private static void TP4(){
        ...
        CCSPfns.tolerateInfeasible = false;
        
        knownOptSol = -29.2; 
        knownOptSolLowerLevel = 3.2;
        lLimit = -60;
        uLimit = 10;
        
        double x1 = x[0], x2 = x[1], y1 = x[2], y2 = x[3], y3 = x[4];
        
        curFitness = -8*x1-4*x2+4*y1-40*y2-4*y3;
        violation.clear();
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        //lower level
        t[0] = y2+y3-y1-1; //<=0
        t[1] = 2*x1-y1+2*y2-0.5*y3-1; //<=0
        t[2] = 2*x2+2*y1-y2-0.5*y3-1; //<=0
        
        //upper level
        //nothing only parametric
        ...
        curLowerLevelFitness = x1+2*x2+y1+y2+2*y3;
        ...
        //4 parameteric constraints (variable level constraints)
        for (int i = 0; i < x.length; i++) {
            consFn.add(new Constraint(x[i])); 
        }
     ...
     * <code>
     * total vars (n) = 4; Upper 2, lower 2<BR>
     * total constraints = 4 parametric and 2 functional <BR>
     * -&infin;&le;x<sub>i</sub>&le;+&infin;, i = 0,1<BR>
     * 0&le;x<sub>j</sub>&le;+&infin;, j = 2,3<BR>
     * x1 = x[0], x2 = x[1], y1= x[2], y2 = x[3]<BR>
     * </code>
     */
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        Double ULxT[][]  = MyMath.matrixTranspose(ULx);
        Double LLyT[][]  = MyMath.matrixTranspose(LLy);

        Double txx[][]=MyMath.matrixMultiplication(ULxT, ULx); //1x1 array
        Double tyy[][]=MyMath.matrixMultiplication(LLyT, LLy); //1x1 array
         
        curFitness = r*txx[0][0]-3*y1-4*y2+0.5*tyy[0][0];
        violation.clear();
        ...
        Double h[][] = {{1.0, 3.0}, {3.0, 10.0}};
        Double b[][] = {{-1.0, 2.0}, {3.0, -3.0}};
        
        Double tmp[][] = MyMath.matrixMultiplication(LLyT, h);//[1x2] x  [2x2] = [1x2]
        tmp = MyMath.matrixMultiplication(tmp,LLy); //[1x2] x [2x1] = [1x1]
        
        curLowerLevelFitness = 0.5*tmp[0][0];
        tmp = MyMath.matrixMultiplication(b,ULx); //b(x)=[2x2] x [2x1] = [2x1]
        tmp = MyMath.matrixTranspose(tmp);//[1x2]
        tmp = MyMath.matrixMultiplication(tmp,LLy); //[1x2] x [2x1] = [1x1]
        curLowerLevelFitness = curLowerLevelFitness-tmp[0][0];
     ...
     * 
     * <code>
     * total vars (n) = 3; Upper 1, lower 2<BR>
     * total constraints = 3 parametric and 4 functional <BR>
     * 0&le;x<sub>0</sub><BR>
     * 0&le;x<sub>j</sub>, j = 1,2<BR>
     * x1 = x[0], y1 = x[1], y2 = x[2]<BR>
     * </code>
     */
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        double x1 = x[0], y1 = x[1], y2 = x[2];
                
        curFitness = Math.pow(x1-1,2)+2*y1-2*x1;
        violation.clear();
        
        double t[] = new double[5];
        //lower level
        t[0] = 4*x1+5*y1+4*y2-12; //<=0
        t[1] = 4*y2-4*x1-5*y1+4; //<=0
        t[2] = 4*x1-4*y1+5*y2-4; //<=0
        t[3] = 4*y1-4*x1+5*y2-4; //<=0
        
        //upper level
        //nothing only parametric
        
        curLowerLevelFitness = Math.pow(2*y1-4,2)+Math.pow(2*y2-1, 2)+x1*y1;
     ...
     * total vars (n) = 4<BR>
     * total constraints = 4 parametric and 3 functional <BR>
     * 0&le;x<sub>i</sub>, i = 0,1<BR>
     * 0&le;x<sub>j</sub>, j = 2,3<BR>
     * x1 = x[0], x2 = x[1], y1 = x[2], y2 = x[3];<BR>
     * </code>
     */
    private static void TP3(){
        ...
        knownOptSol = -18.6787; 
        knownOptSolLowerLevel = -1.0156;
        lLimit = -40;
        uLimit = 20;
        
        curFitness = -Math.pow(x1,2)-3*Math.pow(x2,2)-4*y1+Math.pow(y2,2);
        violation.clear();
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        double t[] = new double[4];
        //lower level
        t[0] = -3-Math.pow(x1,2)+2*x1-Math.pow(x2,2)+2*y1-y2; //<=0
        t[1] = 4 - x2-3*y1+4*y2; //<=0
        
        //upper level
        t[2] = Math.pow(x1,2)+2*x2-4; //<=0
        
        curLowerLevelFitness = 2*Math.pow(x1,2)+Math.pow(y1, 2)-5*y2;
     ...
     * <code>
     * total vars (n) = 4; Upper 2, lower 2<BR>
     * total constraints = 4 parametric and 7 functional <BR>
     * 0&le;x<sub>i</sub>&le;50, i = 0,1<BR>
     * -10&ge;x<sub>j</sub>&ge;20 [OR relationship], j = 2,3<BR>
     * x1 = x[0], x2 = x[1], y1= x[2], y2 = x[3]<BR>
     * </code>
     */
    private static void TP2(){
        ...
        curFitness = 2*x1+2*x2-3*y1-3*y2-60;
        violation.clear();
        ...
        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
         ...
         int OR[][] = new int[2][2]; //make it global
        //upper level constraints
        

        //lower level constraints.
        // ORs should always be first ones
        t[0] = y1+10; //<=0; .. OR
        t[1] = 20-y1;//<=0;
        OR[0][0] = 0; //idx
        OR[0][1] = 1; //idx
...
////        t[4] = -x[3]-10; //<=0
        ...
        for (int i = 0; i < OR.length; i++) {
            if(consFn.get(OR[i][0]).val >= 0.0 || 
            consFn.get(OR[i][1]).val >= 0.0){ //dt - ...
                CSPsols.get(lastIdx).set(2*i+0, (ArrayList<Double>)vals.clone()); //??? dummy one. same x and diff y  
                CSPsols.get(lastIdx).set(2*i+1, (ArrayList<Double>)vals.clone()); //??? dummy one. same x and diff y  
            }else{ //can  more refine to pin point if only 1/2 isViolated.
                consFn.get(2*i+0).isViolated = true;
                violation.add(2*i+0);
                consFn.get(2*i+1).isViolated = true;
                violation.add(2*i+1);
            } 
        }
        
        for (int i = OR.length*2; i < consFn.size(); i++) {
            if(consFn.get(i).val >= 0.0){ //dt - ...
                CSPsols.get(lastIdx).set(i, (ArrayList<Double>)vals.clone()); //??? dummy one. same x and diff y  
            }else{
                consFn.get(i).isViolated = true;
    ...
    //bi-level single objective optimization problems 
    /**
     * Minimize <code>F(x,y) = (x<sub>1</sub>-30)<sup>2</sup>+(x<sub>2</sub>-20)<sup>2</sup>-20y<sub>1</sub>+20y<sub>2</sub></code><BR>
     * ......<code>(x,y)</code><BR>
     * s.t. <BR>
     * <code>n = 2, m = 2<BR>
     * y &isin; argmin{f(x,y)=(x<sub>1</sub>-y<sub>1</sub>)<sup>2</sup>+(x<sub>2</sub>-y<sub>2</sub>)<sup>2</sup>}<BR>
     * .......(y)<BR>
     * where 0&le;y<sub>1</sub>&le;10, i= 1,2<BR>
     * x<sub>1</sub>+2x<sub>2</sub>&ge;30, x<sub>1</sub>+x<sub>2</sub>&le;25, x<sub>2</sub>&le;15 
     * </code>
     */
    private static void TP1(){
        ...
        vals.toArray(x);
        // total vars (n) = 4
        // total constraints = 4; 3 normal + 1 lower level COP
        // -inf<=xi<=+inf, i = 0,1
        // 0<=xj<=10, j = 2,3
        //x1 = x[0]; x2 = x[1]; y1 = x[2]; y2 = x[3]
...
//        knownOptSolDP = (int)Math.ceil(minDP+(maxDP - minDP)*1.0*(CspProcess.getDyanmicTime())/(userInput_.maxDynamicTime-1));
        ...
        curFitness = Math.pow(x[0]-30,2)+Math.pow(x[1]-20,2.0)-20*x[2]+20*x[3];
        violation.clear();
...
//        t[2] = x[1]-15;//<=0
        //lower level constraints.
        curLowerLevelFitness = Math.pow(x[0]-x[2],2)+Math.pow(x[1] - x[3], 2);
...
//        final int total_divisions = Math.abs(maxPow-minPow)+1;
//        
//        int slotSize = maxCSPsolsSize/total_divisions;//need floor int
//        int grp = 0;
//        for (int ii = 1; ii < total_divisions; ii++) {
//            if(lastIdx>slotSize*ii){
//                grp++;
//                continue;
//            }else{
//                break;
...
//    }
    ...
    private static void G6(final int tm){
     ...
     private static void G3(){
        ...
        int n = userInput_.getTotalDecisionVars();
...
//        double dt = 1.0E-1;       
        ...
        for(int i = 0; i< t.length; i++){
            if(dt - Math.abs(consFn.get(i).val) >= 0.0){ //??? -ve??. double check...
//                if(CSPsols.size() != maxCSPsolsSize-1 && CSPsols.get(lastIdx).get(ii).isEmpty())
                    CSPsols.get(lastIdx).set(i, (ArrayList<Double>)vals.clone());   
            }else{
                consFn.get(i).isViolated = true;
