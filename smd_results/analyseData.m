...
%         fprintf('proccessing file %s\n',files(i).name);
        ...
        probNo = result{1}(1,1)-10; % -10 is done because smd starts with 11 and my program in netbeans uses TP11 as smd 1 i.e. TP11-10 = smd1gt6ftrde4321
        ...
        if max(size(Bests))<probNo
            Bests{tmpBest(1)-10} = tmpBest; 
        else
            Bests{tmpBest(1)-10}(end+1,:) = tmpBest; 
        end
