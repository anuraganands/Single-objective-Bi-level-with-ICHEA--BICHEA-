...
function analyseData()
    ...
    finalVals = cell(4,10); %EOF, OF, Of, EOf for 10 problems
...
%         fprintf('proccessing file %s\n',files(i).name);
        ...
        ending = [124 249 374 499];
        for con = 1:4
            rowids = find(result{3}== ending(con));

            b = [mean(result{4}(rowids));mean(result{5}(rowids))];
            finalVals{con, probNo} = [finalVals{con,probNo} b]; %F
        end
    ...
    for i = 1:total
        ...
        vF = var(Bests{i}(:,4));
        vf = var(Bests{i}(:,5));
        fprintf('%d, %d, %d, %2f, %2f, %d, %d, %2f, %2f, %2f, %2f, %d', tmpBest);
        fprintf(', %2f, %2f\n',vF, vf);       
    end
